...
# Usage: ./run_local_dpo.sh [experiment_name_suffix]
#
# On-policy DPO baseline for Table 5. Uses the SAME rollout budget as GRPO/MOPD
# (n rollouts per prompt), but optimizes a pairwise preference loss instead:
# within each rollout group, verified-success rollouts (chosen) are preferred over
# verified-failure rollouts (rejected). DPO's KL is implicit through the reference
# model (enabled automatically for loss_mode=dpo).
...
# Matched-budget hyperparameters (mirror run_local_grpo.sh / run_local_sdpo.sh).
TRAIN_BATCH_SIZE=32
ROLLOUT_BATCH_SIZE=8          # rollouts per prompt (n); matches GRPO/MOPD
MINI_BATCH_SIZE=32           # * n internally -> full-batch step, like the baselines
LR=${LR:-1e-6}               # preference losses are sensitive; smaller LR than GRPO
SEED=${SEED:-42}
# DPO temperature (implicit-reward scale).
DPO_BETA=${DPO_BETA:-0.1}
PREF_LABEL_SMOOTHING=${PREF_LABEL_SMOOTHING:-0.0}
# Qwen3-4B is the model reported in Table 5.
...
# NOTE on multi-GPU: DPO pairs are formed within a rollout group. Groups are kept
# contiguous (balance_batch=False) and sized to one micro-batch, but with >1 GPU a
# group can be sharded across data-parallel ranks, yielding fewer pairs per rank.
# For an exact matched-budget run, prefer a single GPU (or n_gpus that divides the
# per-prompt group cleanly).
...
EXP_NAME="${DATA_PATH##*/}-DPO-train${TRAIN_BATCH_SIZE}-rollout${ROLLOUT_BATCH_SIZE}-beta${DPO_BETA}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
...
ARGS=(
  ...
  "actor_rollout_ref.actor.policy_loss.pref_label_smoothing=$PREF_LABEL_SMOOTHING"
)
...
echo "----------------------------------------------------------------"
echo "Starting Local DPO Training (on-policy, verified-success vs failure pairs)"
echo "Experiment: $EXP_NAME"
echo "Data: $DATA_PATH"
echo "Model: $MODEL_PATH"
echo "Rollout backend: $ROLLOUT_BACKEND | n=$ROLLOUT_BATCH_SIZE | beta=$DPO_BETA | lr=$LR"
