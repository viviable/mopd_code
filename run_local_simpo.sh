...
# Usage: ./run_local_simpo.sh [experiment_name_suffix]
#
# On-policy SimPO baseline for Table 5. Same rollout budget as GRPO/MOPD (n rollouts
# per prompt); within each rollout group, verified-success rollouts (chosen) are
# preferred over verified-failure rollouts (rejected). SimPO is reference-free and
# uses a length-normalized implicit reward with a target margin gamma.
...
LR=${LR:-1e-6}
SEED=${SEED:-42}
# SimPO temperature and target reward margin.
...
# NOTE on multi-GPU: SimPO pairs are formed within a rollout group. Groups are kept
# contiguous (balance_batch=False) and sized to one micro-batch; with >1 GPU a group
# may be sharded across data-parallel ranks (fewer pairs per rank). Prefer 1 GPU for
# an exact matched-budget run.

SUFFIX=${1:-"local_simpo"}
...
EXP_NAME="${DATA_PATH##*/}-SimPO-train${TRAIN_BATCH_SIZE}-rollout${ROLLOUT_BATCH_SIZE}-beta${SIMPO_BETA}-gamma${SIMPO_GAMMA}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
...
ARGS=(
  ...
  "trainer.group_name=SimPO-local"
  ...
  "actor_rollout_ref.actor.policy_loss.loss_mode=simpo"
  "actor_rollout_ref.actor.policy_loss.simpo_beta=$SIMPO_BETA"
  "actor_rollout_ref.actor.policy_loss.simpo_gamma=$SIMPO_GAMMA"
...
echo "----------------------------------------------------------------"
echo "Starting Local SimPO Training (on-policy, verified-success vs failure pairs)"
echo "Experiment: $EXP_NAME"
echo "Data: $DATA_PATH"
echo "Model: $MODEL_PATH"
echo "Rollout backend: $ROLLOUT_BACKEND | n=$ROLLOUT_BATCH_SIZE | beta=$SIMPO_BETA | gamma=$SIMPO_GAMMA | lr=$LR"
